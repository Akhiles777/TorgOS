import { getPlatformAiBriefing, type PlatformDashboardForAi } from "@/server/insights/platformAi";
import { parseBriefing } from "@/server/insights/ai";
import { refreshPlatformBriefingAction } from "./actions";
...
// Асинхронный Server Component, стримится через <Suspense> в page.tsx — тот
// же принцип, что и AiBriefingSection.tsx у владельца: медленный/упавший
// вызов RouterAI не блокирует остальную аналитику.
export async function PlatformBriefingSection({ dashboard }: { dashboard: PlatformDashboardForAi }) {
  const briefing = await getPlatformAiBriefing(dashboard);

  if (!briefing) {
    // Ключ не задан, сеть недоступна, визитов за окно нет — тихо ничего не
    // показываем. Честные правила выше работают независимо от ИИ.
    return null;
  }
    ...
    <section className="mb-6">
      <div className="flex items-center justify-between gap-2 mb-3">
        <div className="flex items-baseline gap-2">
          <h2 className="text-sm font-semibold text-ink-soft">Сводка от ИИ</h2>
          <span className="text-xs text-ink-soft">
            {briefing.stale ? "прошлая версия, обновление не удалось" : `обновлено ${timeAgo(briefing.generatedAt)}`}
          </span>
        </div>
        <form action={refreshPlatformBriefingAction}>
          <button type="submit" className="text-xs text-ink-soft hover:text-stamp-text underline underline-offset-2">
            Обновить
          </button>
        </form>
      </div>
      <PlatformBriefingBody content={briefing.content} />
    </section>
  );
}

function PlatformBriefingBody({ content }: { content: string }) {
...
export function PlatformBriefingSkeleton() {
  return (
    <section className="mb-6">
