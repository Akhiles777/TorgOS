...
// sonar, а не sonar-pro-search: на живом сравнении (реальные штрихкоды +
// несуществующий) ответы совпали дословно по смыслу, при этом sonar втрое
// дешевле по входу и в 15 раз по выходу и отвечает за 2-3с вместо 5-7с.
// Дорогая pro-search-модель себя тут не окупает: задача узкая — найти карточку
// товара по номеру, глубокое рассуждение не нужно.
const DEFAULT_MODEL = "perplexity/sonar";
// Поиск в сети — несколько секунд на запрос; под параллельной нагрузкой роутер
// отвечает дольше, поэтому запас больше стандартных 25с из routerai.ts.
...
export type BarcodeLookupResult =
  ...
  | { barcode: string; found: true; name: string; category: string; known?: true }
