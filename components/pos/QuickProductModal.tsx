...
export function QuickProductModal({
  ...
  const lookup = async () => {
    ...
    if (res.ok) {
      ...
      // «Не удалось проверить» и «товара нет» — разные вещи: в первом случае
      // кассиру стоит нажать ещё раз, а не вбивать название руками.
      setError(res.retryable ? `${res.error}.` : `${res.error}. Впишите название сами.`);
    }
  };
